...
export class GoGame {
    ...
    getBoardSize(): BoardSize { return this.size; }

    setGameMode(mode: 'TWO_PLAYER' | 'VS_AI') { this.mode = mode; }
    ...
    setKomi(k: number) { this.komi = k; }
    getKomi() { return this.komi; }
    ...
    private findCaptures(row: number, col: number, opponent: Player): Position[] {
        ...
        // Dedup
        return captured.filter((v, i, a) => a.findIndex(t => t.row === v.row && t.col === v.col) === i);
    }
    ...
    private serializeBoard(): string {
        return this.board.map(row => row.map(p => p === 'BLACK' ? 'B' : (p === 'WHITE' ? 'W' : '.')).join('')).join('\n');
    }
    ...
    private applyHandicap() {
        ...
        const points: Record<number, Position[]> = {
            9: [{ row: 2, col: 6 }, { row: 6, col: 2 }, { row: 6, col: 6 }, { row: 2, col: 2 }, { row: 4, col: 4 }],
            13: [{ row: 3, col: 9 }, { row: 9, col: 3 }, { row: 9, col: 9 }, { row: 3, col: 3 }, { row: 6, col: 6 }, { row: 3, col: 6 }, { row: 9, col: 6 }, { row: 6, col: 3 }, { row: 6, col: 9 }],
            19: [{ row: 3, col: 15 }, { row: 15, col: 3 }, { row: 15, col: 15 }, { row: 3, col: 3 }, { row: 9, col: 9 }, { row: 3, col: 9 }, { row: 15, col: 9 }, { row: 9, col: 3 }, { row: 9, col: 15 }]
        };
    ...
    getBoard(): (Player | null)[][] { return this.board; }
    getMoveCount(): number { return this.history.length; }
    getCaptures(): Record<Player, number> { return this.captures; }
    ...
    private notifyStateChange() { this.stateChangeListeners.forEach(l => l(this.gameState)); }
    private notifyMove(m: Move) { this.moveListeners.forEach(l => l(m)); }
    private notifyBoardUpdate() { this.boardUpdateListeners.forEach(l => l()); }
    private notifyTimerUpdate() { this.timerUpdateListeners.forEach(l => l(this.elapsedTime)); }

    onStateChange(l: (s: GameState) => void) { this.stateChangeListeners.push(l); }
    onMove(l: (m: Move) => void) { this.moveListeners.push(l); }
    onBoardUpdate(l: () => void) { this.boardUpdateListeners.push(l); }
    onTimerUpdate(l: (e: number) => void) { this.timerUpdateListeners.push(l); }
